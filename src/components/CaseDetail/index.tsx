...
import { Card, Tabs, Descriptions, Table, Button, Form, Input, InputNumber, Select, Tag, Space, message, List, Avatar, Badge, Breadcrumb, Divider, Progress, Alert } from 'antd';
import type { ColumnsType } from 'antd/es/table';
import { ArrowLeftOutlined, PlusOutlined, PhoneOutlined, UserOutlined, HomeOutlined, CreditCardOutlined, FileTextOutlined, HistoryOutlined, CheckCircleOutlined, ClockCircleOutlined, DollarOutlined } from '@ant-design/icons';
import CallButton from '../CallCenter/CallButton';
import AddContactModal, { Contact } from '../CallCenter/AddContactModal';
...
const CaseDetail: React.FC<CaseDetailProps> = ({ caseId, onBack }) => {
  ...
  const caseData: Case = {
    ...
    lastUpdateTime: '2024-03-30 14:30:00',
  };
  ...
  const allContacts = [
    // 借款人本人
    { id: 'self', name: basicInfo.name, relationship: '本人', phone: basicInfo.phone, addedBy: '系统', addedAt: caseData.createTime, source: '系统自带' },
    // 工作电话
    { id: 'work', name: basicInfo.name, relationship: '本人', phone: workInfo.workPhone, addedBy: '系统', addedAt: caseData.createTime, source: '系统自带' },
    // 紧急联系人
  ...
  return (
    <div>
      ...
      <Breadcrumb style={{ marginBottom: 16 }}>
        ...
        <Breadcrumb.Item>案件列表</Breadcrumb.Item>
      ...
      <Card style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            ...
            <p style={{ color: '#666', marginTop: 4 }}>最后更新: {caseData.lastUpdateTime}</p>
          </div>
          <div style={{ display: 'flex', gap: 12 }}>
            <Badge status="processing" text={caseData.status} style={{ fontSize: 14 }} />
            <Badge status={caseData.overdueDays > 30 ? 'error' : caseData.overdueDays > 15 ? 'warning' : 'default'} text={`逾期 ${caseData.overdueDays} 天`} style={{ fontSize: 14 }} />
            <Badge status="default" text={caseData.stage} style={{ fontSize: 14 }} />
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
          <div style={{ textAlign: 'center', padding: 16, backgroundColor: '#f6ffed', borderRadius: 8, border: '1px solid #b7eb8f' }}>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#389e0d' }}>¥{caseData.amount.toLocaleString()}</div>
            <div style={{ color: '#666', marginTop: 4 }}>逾期金额</div>
          </div>
          <div style={{ textAlign: 'center', padding: 16, backgroundColor: '#fff2e8', borderRadius: 8, border: '1px solid #ffd591' }}>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#fa8c16' }}>{caseData.overdueDays}</div>
            <div style={{ color: '#666', marginTop: 4 }}>逾期天数</div>
          </div>
          <div style={{ textAlign: 'center', padding: 16, backgroundColor: '#e6f7ff', borderRadius: 8, border: '1px solid #91d5ff' }}>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#1890ff' }}>{caseData.assignedTo}</div>
            <div style={{ color: '#666', marginTop: 4 }}>负责催收员</div>
          </div>
          <div style={{ textAlign: 'center', padding: 16, backgroundColor: '#f9f0ff', borderRadius: 8, border: '1px solid #d3adf7' }}>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#722ed1' }}>{caseData.createTime.split(' ')[0]}</div>
            <div style={{ color: '#666', marginTop: 4 }}>立案时间</div>
          </div>
        </div>
      </Card>
      ...
      <Tabs activeKey={activeTab} onChange={setActiveTab} style={{ marginBottom: 24 }}>
        <Tabs.TabPane tab="客户信息" key="basic">
          <Card style={{ marginBottom: 16 }}>
            <Descriptions title="个人基本信息" column={2} bordered>
              ...
              <Descriptions.Item label="电话">
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  {basicInfo.phone}
                  <CallButton phone={basicInfo.phone} contactName={basicInfo.name} relationship="本人" />
                </div>
              </Descriptions.Item>
          ...
          <Card style={{ marginBottom: 16 }}>
            <Descriptions title="工作信息" column={2} bordered>
              ...
              <Descriptions.Item label="工作电话">
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  {workInfo.workPhone}
                  <CallButton phone={workInfo.workPhone} contactName={basicInfo.name} relationship="本人" />
                </div>
              </Descriptions.Item>
          ...
          <Card style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0 }}>紧急联系人</h3>
              <Button type="primary" icon={<PlusOutlined />} onClick={() => setAddContactVisible(true)}>
                添加联系号码
              </Button>
            </div>
            <Table
              ...
              columns={[
                ...
                { title: '电话', dataIndex: 'phone', key: 'phone', render: (phone: string, record: any) => (
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    {phone}
                    <CallButton phone={phone} contactName={record.name} relationship={record.relationship} />
                  </div>
                )},
        ...
        <Tabs.TabPane tab="订单信息" key="order">
          <Card style={{ marginBottom: 16 }}>
          ...
          <Card>
            <h3 style={{ marginBottom: 16 }}>账单信息</h3>
            <Table
              dataSource={billInfo}
              columns={[
        ...
        <Tabs.TabPane tab="VA码管理" key="va">
          <Card style={{ marginBottom: 16 }}>
            <h3 style={{ marginBottom: 16 }}>当前有效VA码</h3>
            <Table
          ...
          <Card>
            <h3 style={{ marginBottom: 16 }}>操作</h3>
            {newVARequest ? (
        ...
        <Tabs.TabPane tab="申请管理" key="apply">
          <Card style={{ marginBottom: 16 }}>
            <h3 style={{ marginBottom: 16 }}>减免申请</h3>
            {reductionRequest ? (
          ...
          <Card>
            <h3 style={{ marginBottom: 16 }}>停催申请</h3>
        ...
        <Tabs.TabPane tab="催收记录" key="records">
          <Card style={{ marginBottom: 16 }}>
            <h3 style={{ marginBottom: 16 }}>添加催收记录</h3>
          ...
          <Card>
            <h3 style={{ marginBottom: 16 }}>历史催收记录</h3>
            <List
                  ...
                  <List.Item.Meta
                    ...
                    title={
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span>
                          <strong>{record.collector}</strong> - {record.type}
                        </span>
                        <Badge status={record.result === '已还款' ? 'success' : record.result === '已承诺还款' ? 'processing' : 'default'} text={record.result} />
                      </div>
                    }
                    description={
                      <div>
                        <p style={{ marginBottom: 8 }}>{record.content}</p>
                        <p style={{ color: '#999', fontSize: 12, textAlign: 'right' }}>{record.createTime}</p>
                      </div>
                    }
                  />
                </List.Item>
