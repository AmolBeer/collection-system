...
const defaultData: SMSTemplate[] = [
  { id: 'S001', name: '还款提醒短信', providerId: 'SP001', providerName: 'Twilio SMS Gateway',
    content: '尊敬的{{customerName}}，您的{{productName}}应还金额为{{amount}}元，到期日为{{dueDate}}，请及时还款。',
    status: 'active', createTime: '2024-04-01 10:00:00', description: '还款日前1天提醒' },
  { id: 'S002', name: '逾期催收短信', providerId: 'SP002', providerName: 'Infobip SMS',
    content: '【催收提醒】{{customerName}}您好，您的贷款已逾期{{overdueDays}}天，应还金额{{amount}}元，请尽快处理以免影响信用记录。',
    status: 'active', createTime: '2024-04-02 11:30:00', description: '逾期阶段催收' },
  { id: 'S003', name: '高逾期专项短信', providerId: 'SP001', providerName: 'Twilio SMS Gateway',
    content: '{{customerName}}，您的贷款已严重逾期{{overdueDays}}天，欠款{{amount}}元。请于3日内还清，否则将采取法律措施。',
    status: 'inactive', createTime: '2024-04-07 10:00:00', description: '逾期30天以上专项' },
];

const variables = ['{{customerName}}', '{{amount}}', '{{dueDate}}', '{{productName}}', '{{overdueDays}}', '{{phoneNumber}}'];

const SMSTemplateMgmt: React.FC = () => {
  const [templates, setTemplates] = useState<SMSTemplate[]>(defaultData);
  ...
  const handleEdit = (r: SMSTemplate) => { setEditing(r); form.setFieldsValue({ ...r, status: r.status === 'active' }); setModalVisible(true); };
  const handleDelete = (id: string) => { setTemplates(p => p.filter(t => t.id !== id)); message.success('已删除'); };
  const handleCopy = (r: SMSTemplate) => { setTemplates(p => [...p, { ...r, id: `S${Date.now().toString().slice(-5)}`, name: `${r.name} - 副本`, status: 'inactive' as const, createTime: nowStr() }]); message.success('已复制'); };
  ...
  const handleSubmit = async () => {
    ...
    if (editing) {
      ...
      setTemplates(p => [...p, { id: `S${Date.now().toString().slice(-5)}`, ...values, providerName: provider?.name || '', status: values.status ? 'active' : 'inactive', createTime: nowStr() }]);
      message.success('已创建');
    }
  ...
  const columns = [
    ...buildCommonColumns<SMSTemplate>('SMS', { onEdit: handleEdit, onCopy: handleCopy, onDelete: handleDelete, onToggleStatus: handleToggle }),
    {
  ...
  return (
    <Card variant="borderless" title={<span style={{ color: '#0d4f3c', fontWeight: 600 }}>SMS 模板管理</span>}
      ...
      <Modal title={<span style={{ color: '#0d4f3c' }}>{editing ? '编辑' : '创建'} SMS 模板</span>}
        ...
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item name="name" label={<span style={labelStyle}>模板名称</span>} rules={[{ required: true, message: '请输入' }]}>
            <Input placeholder="例如：还款提醒短信" style={{ borderRadius: 6 }} />
          </Form.Item>
          <Form.Item name="providerId" label={<span style={labelStyle}>绑定服务商</span>} rules={[{ required: true, message: '请选择' }]}>
            <Select placeholder="选择服务商" style={{ borderRadius: 6 }} options={getEnabledProvidersByType('SMS').map(p => ({ value: p.id, label: `${p.name} (${p.id})` }))} />
          </Form.Item>
          <Form.Item name="content" label={<span style={labelStyle}>短信内容</span>} rules={[{ required: true, message: '请输入短信内容' }]}
            extra={<span style={{ fontSize: '12px', color: '#9ca3af' }}>点击下方变量可插入到内容末尾</span>}>
            ...
            <Input.TextArea rows={5} placeholder="请输入短信内容" style={{ borderRadius: 6 }} />
          </Form.Item>
...
export default SMSTemplateMgmt;
